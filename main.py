...
def handle_user_message(message):
    ...
    if "search" in keywords:
        ...
        product_name = " ".join([word for word in keywords if word != "buy"]).strip()
        ...
        response = (
            ...
            "🔍 Search for products: 'search [product name]'\n"
            "🛒 Buy a product: 'buy [product name]'\n"
            "💡 Get product suggestions: 'suggest [category]'\n"
            "🔑 Log in to make purchases and track orders.\n"
            "If you have any other questions, feel free to ask!"
        )

    elif "info" in keywords or "details" in keywords:
        product_name = " ".join([word for word in keywords if word not in ["info", "details"]]).strip()
        product = Product.query.filter(Product.name.ilike(f'%{product_name}%')).first()
        if product:
            response = (
                f"📄 Product Details for {product.name}:\n"
                f"🔹 Description: {product.description}\n"
                f"💲 Price: ${product.price}\n"
                f"📦 Stock: {product.stock}\n"
                "Would you like to buy this product? Use 'buy [product name]' to purchase."
            )
        else:
            response = "🤔 Hmm, I couldn't find that product. Could you check the name again?"
